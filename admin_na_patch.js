(function(){
  ...
  window.renderDetails=function(){
    ...
    reportRows.filter(r=>r.status==='done'||r.status==='unknown').forEach(r=>{
      ...
      nas.forEach(n=>{lines.push({r,a:{item:Number(n),type:'na',text:'생산 비가동/해당없음',priority:'',photos:{}}});added=true;});
